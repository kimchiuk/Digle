  ...
  setMainStream((prevMainStream) => {
    ...
    useEffect(() => {
          ...
          janus = new Janus({
            ...
            success: function () {
              janus.attach({
                ...
                // onlocaltrack 이벤트 핸들러 수정
                onlocaltrack: function (track, on) {
                  Janus.debug(" ::: Got a local track :::", track);
                  
                  // 현재 트랙이 비디오 트랙인지 오디오 트랙인지 확인하고 myFeed 상태 업데이트
                  if(track.kind === 'video') {
                    setMyFeed((prevFeed) => ({
                      ...prevFeed,
                      videoTrack: track
                    }));
                    // 처음 설정 시 mainStream에도 같은 트랙 정보 설정
                    if (!mainStream.videoTrack) {
                      setMainStream(prevMainStream => ({
                        ...prevMainStream,
                        videoTrack: track,
                        username: username // 현재 사용자의 이름을 username으로 설정
                      }));
                    }
      ...
      function newRemoteFeed(id, display, audio, video) {
        ...
        janus.attach({
          ...
          // onremotetrack 이벤트 핸들러 내에서
          onremotetrack: function (track, on) {
            console.log("리모트트랙 들어왔습니다", track);
            // 피드 상태를 업데이트
            setFeeds((currentFeeds) => {
              return currentFeeds.map((feed) => {
                if (feed.rfid === remoteFeed.rfid) { // 현재 피드를 찾음
                  // 오디오 또는 비디오 트랙에 따라 해당 피드를 업데이트
                  const updatedFeed = {
                    ...feed,
                    audioTrack: track.kind === 'audio' ? track : feed.audioTrack,
                    videoTrack: track.kind === 'video' ? track : feed.videoTrack,
                  };
                  return updatedFeed;
                }
                return feed;
    ...
    useEffect(() => {
      console.log("내마이피드가 변경습니다요:", tracks);
    }, [tracks]);
    ...
    useEffect(() => {
      console.log("메인스트림이 설정됬습니다요:", tracks);
    }, [tracks]);
    ...
    const renderRemoteVideos = feeds.map((feed) => {
      return (
        <div key={feed.rfid} onClick={() => handleMainStream(feed,feed.rfdisplay)} className="flex w-[357px] h-[268px] m-2">
          <Video
  ...
  />
            ...
            <div className="flex justify-end items-center mt-2 min-w-[700px]">
              ...
              <button onClick={handleVideoActiveClick} className="mr-2">
                {activeVideo ? (
                  ...
                  <img className="min-w-5 h-5" src={camOff} alt="카메라 끄기" />
                )}
              </button>
        ...
        <div className="w-full min-w-[787px] bg-black rounded-md mt-2 mb-10">
          ...
          <div onClick={() => handleMainStream(myFeed, username)}>
    {myFeed && (
      <Video
        className="w-[357px] h-[268px] mt-2 mx-2"
        audioTrack={myFeed.audioTrack}
        videoTrack={myFeed.videoTrack}
        username={username}
        muted={true}
      />
    )}
