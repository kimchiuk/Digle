...
function RoomList() {
    ...
    useEffect(() => {
        axios.get('https://localhost:8000/rooms/list')
            .then(response => {
    ...
    const handleDeleteRoom = (room_id) => {
        axios.post(`https://localhost:8000/rooms/${room_id}/destroy`)
            .then(() => {
    ...
    const handleShowParticipants = async (room_id) => {
        ...
        try {
            const response = await axios.get(`https://localhost:8000/rooms/${room_id}/participants`);
            console.log(response);
            const fetchedParticipants = response.data;
            
            setParticipants(prevParticipants => ({
                ...prevParticipants,
                [room_id]: fetchedParticipants.length > 0 ? fetchedParticipants : "NoParticipants"
            }));
        } catch (error) {
