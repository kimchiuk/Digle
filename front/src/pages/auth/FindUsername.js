...
const FindUsername = () => {
  ...
  const findUsername = async () => {
    ...
    try {
      ...
      // 에러 발생 시 메시지를 설정합니다.
      setError(err.response?.data?.message || '잘못 입력하였거나 존재하지 않는 E-mail입니다.');
    } finally {
    ...
    <div className='flex flex-col items-center justify-center space-y-4 pt-20'>
          ...
          <div className="flex flex-col items-center space-y-2">
            ...
            <button
              ...
              disabled={loading}
              className={`px-4 py-2 border rounded w-full max-w-xs ${loading ? 'bg-gray-300' : 'bg-blue-500 text-white'}`}
            >
              {loading ? '로딩 중...' : '아이디 찾기'}
            </button>
            {username && !error && (
              <p className="text-green-500">회원님의 아이디는 <strong>{username}</strong> 입니다.</p>
            )}
